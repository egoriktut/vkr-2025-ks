...
from app.user.schemas import UserBase, UserHistory, UserChangeCredentials
...
class UserService:
    ...
    @staticmethod
    def change_credentials(db: Session , token: str, new_credentials: UserChangeCredentials):
        user = db.query(User).filter_by(token=token).first()
        if not user:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
